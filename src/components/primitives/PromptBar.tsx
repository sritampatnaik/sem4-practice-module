...
/* Beautiful UI PromptBar keeps a self-running demo loop in effects. */
...
const COMMANDS = [
  { key: "quiz", name: "/quiz", desc: "Five MCQs on this topic" },
  { key: "flashcards", name: "/flashcards", desc: "A short revision deck" },
  { key: "syllabus", name: "/syllabus", desc: "Check this against the syllabus" },
  { key: "worked-example", name: "/worked-example", desc: "Show the working step by step" },
  { key: "summarise", name: "/summarise", desc: "Digest the thread so far" },
];

const MODELS = [
  { key: "mets-desk", name: "METS desk", tag: "Flagship" },
  { key: "auto-route", name: "Auto route", tag: "Default" },
  { key: "math", name: "Math", tag: "Specialist" },
];

const FILES = ["h2-calculus.pdf", "kinematics.pdf", "redox-notes.pdf"];
const DICTATION = "Explain chemical bonding for O-Level";
...
const AUTO_STEPS: {
  ...
  { draft: "", connect: false, model: "auto-route", hold: 1100 },
  ...
  { draft: "", model: "mets-desk", hold: 2400 },
  { draft: "", hold: 900 },
];
...
export default function PromptBar({
  ...
  const selectModel = (next: (typeof MODELS)[number]) => {
    setModel(next);
    setModelOpen(false);
    if (next.key === "mets-desk") celebrate();
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* send — tactile square (round in the pill variant); becomes stop while the tutor is writing */}
          <button
            type="button"
